(() => {
  ...
  const state = { financeState: 'All', financeInstitutionId: '', exposureInstitutionId: '' };
  ...
  function exposureRows() {
    return filterInstitutions(DATA.institutions, { year: +$('inst-year').value, control: $('inst-control').value, scope: $('inst-scope').value, institutionId: state.exposureInstitutionId }).filter(row => row.mapX != null && row.mapY != null);
  }
